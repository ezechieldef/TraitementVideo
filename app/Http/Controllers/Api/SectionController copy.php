...
class SectionController extends Controller
{
    ...
    public function index(Video $video): JsonResponse
    {
        ...
        // Optional filtering by language (base code like "fr", "en")
        $langParam = (string) request()->input('langue', '');
        $baseLang = $langParam !== '' ? explode('-', $langParam)[0] : '';

        $query = $video->sections()->withCount('resumes');
        if ($baseLang !== '') {
            $query->where('langue', $baseLang);
        }

        /** @var Collection<int, Section> $sections */
        $sections = $query->orderBy('ordre')->orderBy('debut')->get();
